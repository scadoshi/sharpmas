...
OUT="${OUT:-demo/out}"
RAW="/tmp/sharpmas-demo-raw.mov"
DISPLAY_ID="${DISPLAY_ID:-1}"
SETTLE="${SETTLE:-1.5}"    # seconds of quiet head and tail around each act

name_for() {
  case "$1" in
    1) echo "01-run-a-day" ;;      2) echo "02-independent-solver" ;;
    3) echo "03-sum-types-in-csharp" ;; 4) echo "04-same-tool-two-languages" ;;
  esac
}
...
main() {
  local acts=("$@"); [ ${#acts[@]} -eq 0 ] && acts=(1 2 3 4)
  mkdir -p "$OUT"
  build_layout
  open_window
  for a in "${acts[@]}"; do
